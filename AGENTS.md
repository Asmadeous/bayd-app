...
B.A.Y.D is a Rails 8.1 API (primary, repo root) with a Next.js 16 client in
`client/`. Ruby 4.0.2, PostgreSQL + PostGIS, Solid Queue/Cache (no Redis).

**Backend (Rails, repo root)**
- Dev (both servers): `bin/dev` (Rails :3000 + Next :3003, via `Procfile.dev`)
- Rails only: `bin/rails server -p 3000`
- Test: `bundle exec rspec` (RSpec; ~40 spec files in `spec/`)
...
- Verify: `bin/ci` (runs setup + rubocop + bundler-audit + brakeman — **does NOT
  run rspec**; run `bundle exec rspec` separately as the logic gate)

**Frontend (`client/`)**
- Dev: `npm run dev` (Next.js on :3003)
- Build: `npm run build`
- Typecheck: `npx tsc --noEmit`
- Lint: `npm run lint` (eslint)

The `test` command above (`bundle exec rspec`) is the opt-in switch: tests are a
gate for logic-bearing steps. See `blueprint/context/coding-standards.md`.
